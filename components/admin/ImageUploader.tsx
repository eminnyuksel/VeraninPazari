...
type UploadResult = { success: true; url: string } | { success: false; message: string };
...
export function ImageUploader({ label, value, onChange, multiple = false, max = 1, onRemove, uploadAction = uploadProductImage }: { label: string; value: string[]; onChange: (urls: string[]) => void; multiple?: boolean; max?: number; onRemove?: (url: string) => void; uploadAction?: UploadAction }) {
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    const invalidType = selected.find((file) => !acceptedTypes.has(file.type));
    if (invalidType) {
      toast.error("Yalnızca JPG, PNG, WebP veya AVIF yüklenebilir.");
      return;
    }
    const oversized = selected.find((file) => file.size > maxImageSize);
    if (oversized) {
      toast.error(`${oversized.name} 4 MB sınırını aşıyor.`);
      return;
    }
    setUploading(true);
    try {
  ...
  return (
    <div>
      ...
      {value.length < max && <button type="button" disabled={uploading} onClick={() => inputRef.current?.click()} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void uploadFiles(event.dataTransfer.files); }} className={`admin-upload-zone grid min-h-32 w-full place-items-center rounded-xl border-2 border-dashed p-5 text-center transition disabled:cursor-wait disabled:opacity-70 ${dragging ? "border-emerald-600 bg-emerald-50 dark:bg-emerald-950/30" : "border-slate-300 bg-slate-50 hover:border-emerald-500 dark:border-slate-700 dark:bg-slate-900"}`}><span>{uploading ? <LoaderCircle className="mx-auto animate-spin text-emerald-700" size={28} /> : value.length ? <ImagePlus className="mx-auto text-slate-400" size={28} /> : <UploadCloud className="mx-auto text-slate-400" size={30} />}<strong className="mt-2 block text-sm">{uploading ? "Yükleniyor…" : "Sürükleyin veya görsel seçin"}</strong><small className="text-xs text-slate-400">JPG, PNG, WebP, AVIF • En fazla 4 MB</small></span></button>}
