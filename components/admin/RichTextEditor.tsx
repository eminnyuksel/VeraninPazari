...
export function RichTextEditor({ initialValue, onChange }: { initialValue: string; onChange: (value: string) => void }) {
  ...
  return (
    <div className="admin-rich-editor overflow-hidden rounded-xl border border-slate-200 bg-white focus-within:border-emerald-600 focus-within:ring-4 focus-within:ring-emerald-600/10 dark:border-slate-700 dark:bg-slate-900">
      <div className="admin-editor-toolbar flex gap-1 border-b border-slate-200 p-2 dark:border-slate-700">{commands.map(({ command, label, icon: Icon }) => <button key={command} type="button" title={label} aria-label={label} onClick={() => execute(command)} className="grid size-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800"><Icon size={16} /></button>)}</div>
